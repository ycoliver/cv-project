...
object correspondences. A qualitative matching example with a frozen,
LoRA-tuned ViT-S/16 (Fig.~\ref{fig:teaser}) illustrates this
directly --- mutual-nearest-neighbor correspondences computed solely
on patch tokens recover the underlying geometry of a held-out NAVI
pair without any matching-specific architecture.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../presentation/result/teaser_matching_lora.png}
\caption{\textbf{Local feature matching with LoRA-tuned DINOv3 patch
tokens.} Mutual-nearest-neighbour correspondences (top-60 by cosine
score, colour-coded green$=$high to red$=$low) on a held-out NAVI pair,
using ViT-S/16 patch features from our LoRA-tuned backbone. Despite
keeping all 22M backbone parameters frozen and training only $0.06\%$
of them as rank-4 LoRA adapters on Q/V, MNN over patch tokens already
yields visually clean, geometrically consistent matches. The paper
asks how much of this gain comes from genuine feature movement and
how much from a low-rank re-alignment of an essentially unchanged
representation.}
\label{fig:teaser}
\end{figure}
